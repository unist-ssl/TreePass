...
#include <cassert>
#include <cstdint>
// key_type / value_type are defined in key_generator.h as std::string.
// The previous `using key_type=char*;` here was effectively dead because
// key_generator.h used `#define key_type std::string` to override; that
// macro is now `using`, which would conflict with a redeclaration here.

// [YANG] updated by RocksDB version, 2024.12.08
...
void CompressibleString(Random* rnd, double compressed_fraction, size_t len,
                         std::string* dst) {
 ...
 public:
  RandomGenerator() : data_(), pos_() {
    ...
    while (data_.size() < 1048576) {
      ...
      CompressibleString(&rnd, 1, 100, &piece); // hj: value will be not compressed
      data_.append(piece);
    }
    pos_ = 0;
  }
  ...
  char* Generate(size_t len) {
    //std::cerr << "data: " << data_ << ", pos_: " << pos_ << ", len: " << len << ", datasize : " << data_.size() << std::endl;
    if (pos_ + len > data_.size()) {
      pos_ = 0;
      assert(len < data_.size());
    }
    pos_ += len;
    //std::cerr << " pos update, pos: " << pos_  << ", len: " << len << std::endl;
