...
typedef struct splinterdb_config {
   ...
   // EI update (EWMA) sensitivity knobs. Zero leaves treepass defaults in place.
   double      cache_ei_ewma_alpha;          // 0 < a <= 1
   uint64      cache_ei_initial_per_gb_ns;   // initial EI seed per GB cache (ns)
...
/*
 * Wait for all background tasks (compaction, flush, etc.) to complete.
 * Call this after load to ensure the database is in a stable state
