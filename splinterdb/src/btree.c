 ...
 *      more nodes available for the given height. Uses the mini_allocator's
 *      ci_le_ei flag to allocate with max clock when compaction is hot.
 *-----------------------------------------------------------------------------
 */
bool32
            ...
            btree_node     *node)
{
   ...
   if (mini->ci_le_ei) {
      node->page = cache_alloc_compaction_hot(cc, node->addr, type);
   } else {
               ...
               bool32              is_warm)
{
   ...
   node->page = cache_get_with_is_warm(cc, node->addr, TRUE, type, is_warm);
   node->hdr  = (btree_hdr *)(node->page->data);
}
                       ...
                       uint64                *generation) // OUT
{
   ...
   if (child_next.addr != 0) {
      btree_node_get(cc, cfg, &child_next, PAGE_TYPE_MEMTABLE, TRUE);
...
start_over:
   btree_node_get(cc, cfg, &root_node, PAGE_TYPE_MEMTABLE, TRUE);
   uint64 leaf_wait = 1;
   ...
   btree_node_get(cc, cfg, &child_node, PAGE_TYPE_MEMTABLE, TRUE);
...
/*
 *-----------------------------------------------------------------------------
 * Pivot Tracking Helper Functions (ADDR_CNT version)
 *-----------------------------------------------------------------------------
 */

// Return true if SPECIAL is set on this pivot.
static inline bool
pivot_is_cache_resident(const index_entry *entry)
{
   return (btree_pivot_meta_load(entry) & PIVOT_CACHE_RESIDENT) != 0;
}

// Read pivot hit-counter as u64
static inline uint64_t
pivot_ac_load(index_entry *entry)
{
   return (uint64_t)btree_pivot_meta_ac_load(entry);
}

// Increment hit-counter unless SPECIAL is set (relaxed is fine here).
static inline void
pivot_ac_inc(index_entry *entry)
{
   (void)btree_pivot_meta_ac_inc_atomic(entry);
}

// Mark pivot SPECIAL
static inline void
pivot_mark_cache_resident(index_entry *entry)
{
   btree_pivot_meta_mark_cache_resident(entry);
}

// Clear pivot SPECIAL
static inline void
pivot_clear_cache_resident(index_entry *entry)
{
   btree_pivot_meta_clear_cache_resident(entry);
}

// Set pivot count directly
static inline void
pivot_ac_set(index_entry *entry, uint8_t count)
{
   btree_pivot_meta_ac_set(entry, count);
}

// Unified warm decision with pivot tracking logic.
// Returns TRUE if considered WARM (retained); FALSE otherwise.
// Marks parent node as HOT when pivot transitions COLD→WARM.
static inline bool32
btree_update_ac_and_classify_warm(cache *cc, const btree_config *cfg, btree_node *node, index_entry *entry)
{
   // 1) Check was_cached (special bit = "child was in cache before")
   //    If set, clear it and remember for compensation later.
   bool32 was_cached = pivot_is_cache_resident(entry);
   if (was_cached) {
      pivot_clear_cache_resident(entry);
   }

   // 2) Evict interval not ready => treat as warm (EI-gated retention)
   uint64 ei = cache_get_evict_interval(cc);
   if (ei == EI_NOT_READY) {
      pivot_mark_cache_resident(entry);
      return TRUE;
   }

   // 3) Check if page is COLD (entry_time == 0) or WARM (entry_time != 0)
   timestamp init = __atomic_load_n(&node->page->entry_time, __ATOMIC_RELAXED);
   if (init == 0) {
      return FALSE;
   }

   // 4) Compute time since page entered the cache; read current AC.
   //    t_since_entry is the (T_cur − T_entry) term.
   timestamp now            = platform_get_timestamp();
   uint64    t_since_entry  = (uint64)(now - init);
   uint64    ac             = pivot_ac_load(entry);

   // 4a) was_cached: compensate AC to the max value that still classifies
   //     the page as COLD, i.e. max_ac = floor(t_since_entry / EI) − 1.
   if (was_cached && ei > 0) {
      uint64 threshold_ac = t_since_entry / ei;  // floor
      uint64 max_ac = (threshold_ac > 0) ? (threshold_ac - 1) : 0;
      if (max_ac > ac) {
         pivot_ac_set(entry, (uint8_t)(max_ac > 127 ? 127 : max_ac));
         ac = max_ac;
      }
   }

   // 4b) AC == 0 always classifies as COLD.
   if (ac == 0) {
      pivot_ac_inc(entry);
      return FALSE;
   }

   // 5) Warm/cold classification via the reuse-interval test:
   //       RI = (T_cur − T_entry) / AC       and the page is warm iff RI ≤ EI.
   //    We rearrange to avoid the division:
   //       t_since_entry ≤ EI × AC
   //    ri_numerator is t_since_entry promoted to long double;
   //    ei_x_ac is the RHS of the rearranged inequality.
   if (ei == 0) {
      return TRUE;
   }

   long double ri_numerator = (long double)t_since_entry;
   long double ei_x_ac      = (long double)ei * (long double)ac;

   if (ri_numerator <= ei_x_ac) {
      pivot_mark_cache_resident(entry);
      return TRUE;
   } else {
      // COLD: normally increment, but if the counter is saturated,
      // reset all sibling pivots' ACs and re-anchor entry_time instead.
      if (ac >= PIVOT_AC_SATURATION_THRESHOLD) {
         table_index n = btree_num_entries(node->hdr);
         for (table_index j = 0; j < n; j++) {
            index_entry *sibling = btree_get_index_entry(cfg, node->hdr, j);
            pivot_ac_set(sibling, 0);
         }
         __atomic_store_n(&node->page->entry_time,
                          platform_get_timestamp(), __ATOMIC_RELAXED);
      } else {
         pivot_ac_inc(entry);
      }
      return FALSE;
...
btree_lookup_node(cache              *cc,             // IN
                  ...
                  bool32             *out_is_warm)    // OUT (optional)
{
   ...
   node.addr = root_addr;
   btree_node_get(cc, cfg, &node, type, TRUE);
   ...
   for (h = btree_height(node.hdr); h > stop_at_height; h--) {
      ...
      // Pivot tracking: skip if child is already in cache.
      // Save the parent node's warmth BEFORE the pivot update mutates is_warm —
      // is_warm after the update reflects the CHILD subtree, not this node.
      bool32 node_is_warm = is_warm;
      bool32 child_in_cache = FALSE;
      if (type == PAGE_TYPE_BRANCH) {
         child_in_cache = is_warm && cache_present_by_addr(cc, child_node.addr);
         if (!child_in_cache) {
            is_warm = is_warm && btree_update_ac_and_classify_warm(cc, cfg, &node, entry);
         }
      }

      btree_node_get(cc, cfg, &child_node, type, is_warm);
      debug_assert(child_node.page->disk_addr == child_node.addr);

      // Disk read of internal node: reset stale pivot counts
      if (!child_in_cache && h - 1 > stop_at_height
          && type == PAGE_TYPE_BRANCH) {
         table_index n = btree_num_entries(child_node.hdr);
         for (table_index j = 0; j < n; j++) {
            index_entry *sib = btree_get_index_entry(cfg, child_node.hdr, j);
            pivot_ac_set(sib, 0);
         }
      }

      // Cold path: release this node based on its OWN warmth (node_is_warm),
      // not the child subtree's warmth (is_warm after pivot update).
      if (!node_is_warm && type == PAGE_TYPE_BRANCH) {
         cache_unget(cc, node.page);
         node.page = NULL;
         node.hdr  = NULL;
      } else {
...
btree_lookup_node_async(btree_lookup_async_state *state, uint64 depth)
{
        ...
        state->h--)
   {
      ...
      // Save the parent node's warmth BEFORE the pivot update mutates is_warm —
      // is_warm after the update reflects the CHILD subtree, not this node.
      bool32 node_is_warm = state->is_warm;
      state->child_in_cache = FALSE;
      if (state->type == PAGE_TYPE_BRANCH) {
         state->child_in_cache =
            state->is_warm
            && cache_present_by_addr(state->cc, state->child_node.addr);
         if (!state->child_in_cache) {
            state->is_warm =
               state->is_warm
               && btree_update_ac_and_classify_warm(
                  state->cc, state->cfg, &state->node, entry);
         }
      }
      ...
      // Disk read of internal node: reset stale pivot counts
      if (!state->child_in_cache && state->h - 1 > state->stop_at_height
          && state->type == PAGE_TYPE_BRANCH) {
         table_index n = btree_num_entries(state->child_node.hdr);
         for (table_index j = 0; j < n; j++) {
            index_entry *sib =
               btree_get_index_entry(state->cfg, state->child_node.hdr, j);
            pivot_ac_set(sib, 0);
         }
      }

      // Cold path: release this node based on its OWN warmth (node_is_warm),
      // not the child subtree's warmth (state->is_warm after pivot update).
      if (!node_is_warm && state->type == PAGE_TYPE_BRANCH) {
         cache_unget(state->cc, state->node.page);
         state->node.page = NULL;
         state->node.hdr  = NULL;
      } else {
...
btree_lookup_with_ref(cache              *cc,        // IN
                      ...
                      message            *msg,       // OUT
                      bool32             *found,     // OUT
                      bool32             *out_is_warm) // OUT
{
   bool32 is_warm;
   btree_lookup_node(cc, cfg, root_addr, target, 0, type, node, NULL, &is_warm);
   if (out_is_warm) {
      *out_is_warm = is_warm;
   }
             ...
             merge_accumulator *result)    // OUT
{
   ...
   btree_lookup_with_ref(
      cc, cfg, root_addr, type, target, &node, &data, &local_found, &is_warm);
   if (local_found) {
                       ...
                       bool32             *local_found)           // OUT
{
   ...
   bool32 is_warm;
   btree_lookup_with_ref(
      cc, cfg, root_addr, type, target, &node, &local_data, local_found, &is_warm);
   if (*local_found) {
...
btree_iterator_next_leaf(btree_iterator *itor)
{
   ...
   // Sliding window: subtract old page's clock before releasing it
   if (itor->shared_clock_sum) {
      *itor->shared_clock_sum -= itor->last_clock;
   }

   cache_unget(cc, itor->curr.page);
   itor->curr.page = NULL;
   itor->curr.hdr  = NULL;
   itor->curr.addr = next_addr;
   btree_node_get(cc, cfg, &itor->curr, itor->page_type, itor->is_warm);
   itor->idx          = 0;
   itor->curr_min_idx = -1;

   // Sliding window: add new page's clock after acquiring it
   if (itor->shared_clock_sum) {
      itor->last_clock = cache_get_page_clock(cc, itor->curr.addr, NULL);
      *itor->shared_clock_sum += itor->last_clock;
      if (itor->shared_clock_max && itor->last_clock > *itor->shared_clock_max)
         *itor->shared_clock_max = itor->last_clock;
   }
          ...
          && itor->curr.hdr->generation != itor->end_generation)
   {
       ...
       */
      cache_unget(itor->cc, itor->curr.page);
      itor->curr.page = NULL;
      itor->curr.hdr  = NULL;
      btree_iterator_find_end(itor);
      btree_node_get(itor->cc, itor->cfg, &itor->curr, itor->page_type, itor->is_warm);
   }
...
/*
 * Set up sliding window clock accumulation on an already-initialized iterator.
 * Reads the current page's clock and adds it to the shared accumulator.
 * Must be called after btree_iterator_init.
 */
void
btree_iterator_setup_clock_accum(btree_iterator *itor,
                                 uint64         *shared_sum,
                                 uint64         *shared_count,
                                 uint32         *shared_max)
{
   itor->shared_clock_sum   = shared_sum;
   itor->shared_clock_count = shared_count;
   itor->shared_clock_max   = shared_max;
   if (itor->curr.addr != 0 && itor->curr.page != NULL) {
      itor->last_clock = cache_get_page_clock(itor->cc, itor->curr.addr, NULL);
      *shared_sum += itor->last_clock;
      (*shared_count)++;
      if (itor->last_clock > *shared_max)
         *shared_max = itor->last_clock;
   }
}
...
btree_pack_setup_start(btree_pack_req *req)
{
   ...
   // Propagate CI flag to mini_allocator so btree_alloc uses hot alloc
   req->mini.ci_le_ei = req->ci_le_ei;
...
btree_pack_link_extent(btree_pack_req *req,
                       ...
                       uint64          next_extent_addr)
{
   // Normalize all pages in this extent to the same clock value so they reach
   // clock=0 at the same sweep and can be batched in one writeback request.
   // CI<=EI: alloc already set all pages to max clock uniformly -- skip.
   // CI>EI: leaves get avg (can be 0), internal nodes get max.
   if (!req->ci_le_ei) {
      uint32 extent_clock = 0;
      if (height == 0) {
         uint64 clock_sum = 0;
         for (int i = 0; i < req->num_edges[height]; i++)
            clock_sum +=
               cache_get_page_clock(req->cc, req->edge[height][i].addr, NULL);
         extent_clock = (uint32)(clock_sum / req->num_edges[height]);
      } else {
         for (int i = 0; i < req->num_edges[height]; i++) {
            uint32 clk =
               cache_get_page_clock(req->cc, req->edge[height][i].addr, NULL);
      ...
      for (int i = 0; i < req->num_edges[height]; i++) {
         cache_set_page_clock(req->cc, req->edge[height][i].page, extent_clock);
      }
   }
...
btree_pack_create_next_node(btree_pack_req *req, uint64 height, key pivot)
{
   ...
   // CI is set on mini, so btree_alloc will use CI-based clock
   // ci_le_ei propagated to mini, so btree_alloc uses cache_alloc_compaction_hot.
   btree_alloc(req->cc,
               &req->mini,
               height,
               pivot,
   ...
   // CI <= EI: alloc already set max clock via alloc_compaction_hot
   if (req->ci_le_ei) {
      uint32 alloc_clock = cache_get_page_clock(req->cc, new_node.addr, NULL);
      if (height == 0) {
         uint32 input_max = req->input_clock_max;
         uint32 level_val = (input_max > alloc_clock) ? input_max : alloc_clock;
         if (level_val > req->level_clock_max[0])
            req->level_clock_max[0] = level_val;
         req->input_clock_max = 0;
      } else {
         if (req->level_clock_count[height - 1] > 0) {
            req->level_clock_sum[height - 1]   = 0;
            req->level_clock_count[height - 1] = 0;
            req->level_clock_max[height - 1]   = 0;
         }
         if (alloc_clock > req->level_clock_max[height])
            req->level_clock_max[height] = alloc_clock;
      }
      req->level_clock_sum[height] += alloc_clock;
      req->level_clock_count[height]++;
   }

   // Clock inherit: only needed when CI > EI (alloc didn't set max clock)
   if (!req->ci_le_ei
       && req->input_clock_count > 0) {
      uint32 inherit_clock;
      if (height == 0) {
         inherit_clock = (uint32)(req->input_clock_sum / req->input_clock_count);
      } else {
         if (req->level_clock_count[height - 1] > 0) {
            inherit_clock = req->level_clock_max[height - 1];
            req->level_clock_sum[height - 1]   = 0;
            req->level_clock_count[height - 1] = 0;
            req->level_clock_max[height - 1]   = 0;
         } else {
      ...
      uint32 alloc_clock = cache_get_page_clock(req->cc, new_node.addr, NULL);
      // Floor the inherited clock at 1 so a fresh dirty page survives at least
      // one CLOCK sweep (Sec 3.3 "at least 1", like treepass_inherit_clock).
      // Same for leaf and internal; only the inherit VALUE above differs by
      // height (leaf = input-leaf avg, internal = max child). In this CI>EI
      // path alloc_clock is always 1, so an extra max with it is a no-op.
      uint32 final_clock = (inherit_clock < 1) ? 1 : inherit_clock;
      cache_set_page_clock(req->cc, new_node.page, final_clock);

      // Accumulate for this height level
      req->level_clock_sum[height] += final_clock;
      req->level_clock_count[height]++;
      if (height == 0) {
         uint32 input_max = req->input_clock_max;
         uint32 level_val = (input_max > alloc_clock) ? input_max : alloc_clock;
         ...
         if (final_clock > req->level_clock_max[height])
            req->level_clock_max[height] = final_clock;
      }
   }
...
btree_pack_post_loop(btree_pack_req *req, key last_key)
{
   ...
   btree_node_get(cc, cfg, &root, PAGE_TYPE_BRANCH, TRUE);
               ...
               btree_pivot_stats  *stats)
{
   ...
   btree_lookup_node(
      cc, cfg, root_addr, target, 0, PAGE_TYPE_BRANCH, &leaf, stats, NULL);
   bool32 found;
...
/*
 * Collect warmth statistics for a single branch B+-tree.
 * Only visits pages that are currently in cache (no disk I/O).
 * If a page is not in cache, skip it and all its descendants.
 * This avoids polluting the cache during stat collection.
 */
static void
btree_collect_warmth_recursive(cache              *cc,
                               const btree_config *cfg,
                               uint64              addr,
                               page_type           type,
                               uint64             *total_pivots,
                               uint64             *special_count,
                               uint64             *active_count,
                               double             *active_sum,
                               double             *active_sum_sq,
                               double             *combined_sum,
                               double             *combined_sum_sq,
                               uint64             *cached_leaf_pages,
                               uint64             *cached_internal_pages)
{
   // Skip pages not in cache — no disk I/O, no cache pollution
   if (!cache_present_by_addr(cc, addr))
      return;
   ...
   (*cached_internal_pages)++;

   table_index n = node.hdr->num_entries;
   for (table_index k = 0; k < n; k++) {
      index_entry *ie = (index_entry *)((char *)node.hdr + node.hdr->offsets[k]);
      uint8_t meta = btree_pivot_meta_load(ie);
      uint8_t cnt  = (uint8_t)(meta & PIVOT_AC_MASK);
      (*total_pivots)++;
      *combined_sum    += cnt;
      *combined_sum_sq += (double)cnt * cnt;
      if (meta & PIVOT_CACHE_RESIDENT) {
         (*special_count)++;
      } else {
         (*active_count)++;
         *active_sum    += cnt;
         *active_sum_sq += (double)cnt * cnt;
      }

      // Recurse into child (only if also in cache)
      uint64 child_addr = index_entry_child_addr(ie);
      btree_collect_warmth_recursive(cc, cfg, child_addr, type,
         total_pivots, special_count, active_count,
         active_sum, active_sum_sq, combined_sum, combined_sum_sq,
         cached_leaf_pages, cached_internal_pages);
   }
...
void
btree_collect_warmth_stats(cache              *cc,
                           const btree_config *cfg,
                           uint64              root_addr,
                           page_type           type,
                           btree_warmth_stats *out)
{
   memset(out, 0, sizeof(*out));

   double active_sum = 0, active_sum_sq = 0;
   double combined_sum = 0, combined_sum_sq = 0;

   btree_collect_warmth_recursive(cc, cfg, root_addr, type,
      &out->total_pivots, &out->special_count, &out->active_count,
      &active_sum, &active_sum_sq, &combined_sum, &combined_sum_sq,
      &out->cached_leaf_pages, &out->cached_internal_pages);

   // Compute CVs (only from cached pages' pivot meta)
   if (out->active_count > 1) {
      double mean = active_sum / out->active_count;
      double var  = active_sum_sq / out->active_count - mean * mean;
      if (var > 0 && mean > 0)
         out->active_cv = __builtin_sqrt(var) / mean;
   }
   uint64 total = out->total_pivots;
   if (total > 1) {
      double mean = combined_sum / total;
      double var  = combined_sum_sq / total - mean * mean;
      if (var > 0 && mean > 0)
         out->combined_cv = __builtin_sqrt(var) / mean;
   }
}
                  ...
                  bool32        is_left_edge)
{
   ...
   for (idx = 0; idx < node.hdr->num_entries; idx++) {
      if (node.hdr->height == 0) {
         ...
         btree_node_get(cc, cfg, &child, type, TRUE);
                   ...
                   key           target)              // IN
{
   ...
   for (h = node.hdr->height; h > 0; h--) {
      ...
      btree_node_get(cc, cfg, &child_node, type, TRUE);
      btree_node_unget(cc, cfg, &node);
      node = child_node;
   }
