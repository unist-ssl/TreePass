...
typedef struct btree_iterator {
   ...
   bool32     is_warm;       // warm/cold state from lookup path, used for scan

   // Sliding window clock accumulation (for compaction input tracking)
   uint32  last_clock;          // clock value of currently held leaf page
   uint64 *shared_clock_sum;    // pointer to shared sum across iterators
   uint64 *shared_clock_count;  // pointer to shared iterator count
   uint32 *shared_clock_max;    // pointer to shared max across iterators
} btree_iterator;
...
typedef struct btree_pack_req {
   ...
   bool32              ci_le_ei;     // CI < EI: alloc sets max clock, skip inherit
   ...
   // Sliding window clock accumulation (shared across input iterators)
   uint64 input_clock_sum;                       // sum of currently-held pages' clocks
   uint64 input_clock_count;                     // number of tracked iterators
   uint32 input_clock_max;                       // max input clock (reset per leaf)

   // Hierarchical level accumulator (per output tree height)
   uint64 level_clock_sum[BTREE_MAX_HEIGHT];     // accumulated clock per height
   uint64 level_clock_count[BTREE_MAX_HEIGHT];   // node count per height
   uint32 level_clock_max[BTREE_MAX_HEIGHT];     // max clock per height
...
DEFINE_ASYNC_STATE(btree_lookup_async_state, 3,
   ...
   local, uint32,                       h,
   local, bool32,                       found,
   local, bool32,                       is_warm,
   local, bool32,                       child_in_cache,
   local, message,                      msg,
...
// Per-tree warmth statistics (for branch warmth analysis)
// Only counts pages currently in cache (no disk I/O during collection).
typedef struct btree_warmth_stats {
   uint64 total_pivots;         // pivots on cached internal pages
   uint64 special_count;        // SPECIAL pivots (child was cached)
   uint64 active_count;         // active (counting) pivots
   double active_cv;            // CV of active pivot counts
   double combined_cv;          // CV of all pivot counts
   uint64 cached_leaf_pages;    // leaf pages currently in cache
   uint64 cached_internal_pages; // internal pages currently in cache
} btree_warmth_stats;
