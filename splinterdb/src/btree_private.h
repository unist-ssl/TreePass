...
/* One-byte metadata on child_addr[byte0]:
 * byte0: [ SPECIAL(1) | CNT(7) ], bytes1..7: pure address bytes (4KB-aligned)
 */
#  define PIVOT_CACHE_RESIDENT   (0x80u)  // bit7
#  define PIVOT_AC_MASK  (0x7Fu)  // bits[0..6]

/* When any pivot's CNT reaches this threshold, reset all sibling pivots'
 * counts to 0 and re-anchor the parent page's entry_time to now.
 * Override at build time with -DPIVOT_AC_SATURATION_THRESHOLD=N. */
#ifndef PIVOT_AC_SATURATION_THRESHOLD
#   define PIVOT_AC_SATURATION_THRESHOLD  PIVOT_AC_MASK  // 127
#endif
...
/* Pointer helpers to avoid &packed_member directly (child_addr[byte0]) */
static inline uint8_t *
index_entry_child_addr_b0_ptr(index_entry *entry)
{
    // Address of pivot_data.child_addr (byte 0)
    uintptr_t base = (uintptr_t)(void *)entry;
    uintptr_t off  = (uintptr_t)offsetof(index_entry, pivot_data)
                   + (uintptr_t)offsetof(btree_pivot_data, child_addr);
    return (uint8_t *)(void *)(base + off);
}

static inline const uint8_t *
index_entry_child_addr_b0_ptr_const(const index_entry *entry)
{
    uintptr_t base = (uintptr_t)(const void *)entry;
    uintptr_t off  = (uintptr_t)offsetof(index_entry, pivot_data)
                   + (uintptr_t)offsetof(btree_pivot_data, child_addr);
    return (const uint8_t *)(const void *)(base + off);
}

/* Atomic byte ops on meta (byte0) */
static inline uint8_t
btree_pivot_meta_load(const index_entry *entry)
{
    const uint8_t *b0 = index_entry_child_addr_b0_ptr_const(entry);
    return __atomic_load_n(b0, __ATOMIC_RELAXED);
}

static inline uint8_t
btree_pivot_meta_ac_load(const index_entry *entry)
{
    return (uint8_t)(btree_pivot_meta_load(entry) & PIVOT_AC_MASK);
}

static inline void
btree_pivot_meta_mark_cache_resident(index_entry *entry)
{
    uint8_t *b0 = index_entry_child_addr_b0_ptr(entry);
    __atomic_fetch_or(b0, (uint8_t)PIVOT_CACHE_RESIDENT, __ATOMIC_RELAXED);
}

static inline void
btree_pivot_meta_clear_cache_resident(index_entry *entry)
{
    uint8_t *b0 = index_entry_child_addr_b0_ptr(entry);
    __atomic_fetch_and(b0, (uint8_t)~PIVOT_CACHE_RESIDENT, __ATOMIC_RELAXED);
}

/* Set CNT(7) to a specific value (clamped to 0x7F). Preserves SPECIAL bit. */
static inline void
btree_pivot_meta_ac_set(index_entry *entry, uint8_t count)
{
    uint8_t *p   = index_entry_child_addr_b0_ptr(entry);
    uint8_t  old = __atomic_load_n(p, __ATOMIC_RELAXED);
    if (count > PIVOT_AC_MASK) {
        count = PIVOT_AC_MASK;
    }
    for (;;) {
        uint8_t newv = (uint8_t)((old & PIVOT_CACHE_RESIDENT) | count);
        if (__atomic_compare_exchange_n(p, &old, newv, /*weak=*/false,
...
/* Increment CNT(7) unless SPECIAL is set or CNT is already at max.
 * Overflow is handled by the caller (btree_update_ac_and_classify_warm)
 * which resets all sibling counts and re-anchors entry_time. */
static inline uint8_t
btree_pivot_meta_ac_inc_atomic(index_entry *entry)
{
    uint8_t *p   = index_entry_child_addr_b0_ptr(entry);
    uint8_t  old = __atomic_load_n(p, __ATOMIC_RELAXED);

    for (;;) {
        if (old & PIVOT_CACHE_RESIDENT) {
            return old; // sticky SPECIAL
        }
        uint8_t cnt = (uint8_t)(old & PIVOT_AC_MASK);
        if (cnt >= PIVOT_AC_MASK) {
            return old; // saturated, caller handles overflow reset
        }
        uint8_t newv = (uint8_t)((old & PIVOT_CACHE_RESIDENT) | (uint8_t)(cnt + 1));
        if (__atomic_compare_exchange_n(p, &old, newv, /*weak=*/false,
                                        __ATOMIC_RELAXED, __ATOMIC_RELAXED)) {
            return newv;
        }
...
{
    /* child addresses are 4KiB-aligned; low bits may include pivot meta */
    return entry->pivot_data.child_addr & ~0xFFFULL;
}
