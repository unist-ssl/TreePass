...
#define EI_NOT_READY 0

/*
 * Set to 1 to enable per-entry eviction stats (initial_clock, access_count).
 * Adds struct fields to treepass_entry and per-eviction atomic counters.
 * Causes cache line bouncing under high contention (Zipf) — disable for perf.
 */

typedef struct page_handle {
   char  *data;
   uint64 disk_addr;
   _Atomic timestamp entry_time;        // For warm node only (promotion time)
} page_handle;
 ...
 * the global treepass structure.
 */
typedef struct cache_stats {
   uint64 cache_hits[NUM_PAGE_TYPES];
   uint64 cache_misses[NUM_PAGE_TYPES];
   uint64 cache_miss_time_ns[NUM_PAGE_TYPES];
   uint64 cache_hit_time_ns[NUM_PAGE_TYPES];
   uint64 miss_eviction_time_ns[NUM_PAGE_TYPES];
   uint64 miss_io_time_ns[NUM_PAGE_TYPES];
   uint64 miss_finish_time_ns[NUM_PAGE_TYPES];
   uint64 miss_total_time_ns[NUM_PAGE_TYPES];
   uint64 page_writes[NUM_PAGE_TYPES];
   uint64 page_reads[NUM_PAGE_TYPES];
   uint64 discard_clean[NUM_PAGE_TYPES];    // wasted writeback (clean at discard)
   uint64 discard_dirty[NUM_PAGE_TYPES];    // saved writeback (dirty at discard)
   uint64 discard_uncached[NUM_PAGE_TYPES]; // already evicted (written back)
   uint64 prefetches_issued[NUM_PAGE_TYPES];
   uint64 writes_issued;
   uint64 syncs_issued;

   // Timing breakdown (enabled with CACHE_TIMING_BREAKDOWN)
...
typedef page_handle *(*page_alloc_compaction_hot_fn)(cache *cc, uint64 addr, page_type type);
...
typedef page_handle *(*page_get_fn)(cache    *cc,
                                    ...
                                    page_type type,
                                    bool32    is_warm);
typedef page_handle *(*page_get_with_miss_fn)(cache    *cc,
                                              uint64    addr,
                                              bool32    blocking,
                                              page_type type,
                                              bool32    is_warm,
...
typedef bool32 (*cache_present_by_addr_fn)(cache *cc, uint64 addr);
...
typedef uint64 (*get_ei_fn)(const cache *cc);          // Get EI (Eviction Interval)
typedef bool32 (*get_enable_ei_fn)(const cache *cc);        // EI-based decision enabled?
typedef void (*cache_inherit_clock_fn)(cache *cc, uint64 old_addr, uint64 new_addr, uint32 divisor);
typedef uint32 (*cache_get_page_clock_fn)(cache *cc, uint64 addr, timestamp *out_entry_time);
typedef void (*cache_set_page_clock_fn)(cache *cc, page_handle *page, uint32 clock);

typedef struct cache_lookup_timing {
   uint64 total_cache_access_time_ns; // CACHE_TIMING_BREAKDOWN only
   uint64 total_miss_time_ns;         // read I/O time in misses (always)
   uint64 total_non_cache_time_ns;    // CACHE_TIMING_BREAKDOWN only
   uint64 total_accesses;
} cache_lookup_timing;

typedef void (*cache_get_lookup_timing_fn)(cache *cc, cache_lookup_timing *out);
typedef void (*cache_set_in_lookup_fn)(cache *cc, bool32 in_lookup);
...
typedef struct cache_ops {
   ...
   page_alloc_compaction_hot_fn           page_alloc_compaction_hot;
   extent_discard_fn          extent_discard;
   page_get_fn                page_get;
   page_get_with_miss_fn      page_get_with_miss;
   ...
   cache_present_by_addr_fn cache_present_by_addr;
   ...
   get_ei_fn       get_ei;
   get_enable_ei_fn     get_enable_ei;
   cache_inherit_clock_fn inherit_clock;
   cache_get_page_clock_fn get_page_clock;
   cache_set_page_clock_fn set_page_clock;
   cache_get_lookup_timing_fn get_lookup_timing;
   cache_set_in_lookup_fn set_in_lookup;
} cache_ops;
...
/*
 *----------------------------------------------------------------------
 * cache_alloc_compaction_hot
 *
 * Like cache_alloc, but assigns max clock value.
 * Used when CI <= EI (page will be re-accessed before eviction).
 *----------------------------------------------------------------------
 */
static inline page_handle *
cache_alloc_compaction_hot(cache *cc, uint64 addr, page_type type)
{
   return cc->ops->page_alloc_compaction_hot(cc, addr, type);
}
 ...
 *
 * is_warm indicates whether the page access should be considered warm.
 *
 * addr is a byte offset from the beginning of the disk. It should be aligned
 * to cache_page_size().
 *----------------------------------------------------------------------
 */
static inline page_handle *
cache_get_with_is_warm(cache *cc, uint64 addr, bool32 blocking, page_type type, bool32 is_warm)
{
   return cc->ops->page_get(cc, addr, blocking, type, is_warm);
}
 ...
 *
 * Returns a pointer to the page_handle for the page with address addr.
 * Wrapper that calls cache_get_with_is_warm with is_warm=TRUE.
 *
 * If blocking is set, then it blocks until the page is unlocked as well.
 * If blocking is TRUE, always returns with a read lock held.
 * If blocking is FALSE, returns non-NULL if and only if the thread now holds a
...
{
   return cache_get_with_is_warm(cc, addr, blocking, type, TRUE);
}

/*
 * cache_get_with_miss -- same as cache_get but reports cache miss via was_miss.
 */
static inline page_handle *
cache_get_with_miss(cache  *cc,
                    uint64  addr,
                    bool32  blocking,
                    page_type type,
                    bool32 *was_miss)
{
   return cc->ops->page_get_with_miss(cc, addr, blocking, type, TRUE, was_miss);
}
                           ...
                           void                       *callback_arg)
{
   return cc->ops->page_get_async_state_init(
      buffer, cc, addr, type, is_warm, callback, callback_arg);
}
 ...
 * cache_mark_page_hot
 *
 * Mark a page as hot. Used when a child node transitions from cold to
 * warm so the parent node is treated as hot on the next eviction sweep.
 *----------------------------------------------------------------------
 */
static inline void
cache_mark_page_hot(cache *cc, page_handle *page)
{
   return cc->ops->page_mark_hot(cc, page);
}
...
/*
 *-----------------------------------------------------------------------------
 * cache_present_by_addr
 *
 * Returns TRUE if the page at the given address is present in the cache.
 *-----------------------------------------------------------------------------
 */
static inline bool32
cache_present_by_addr(cache *cc, uint64 addr)
{
   return cc->ops->cache_present_by_addr(cc, addr);
}
...
/*
 *-----------------------------------------------------------------------------
 * cache_get_evict_interval
 *
 * Returns the evict interval threshold for the cache.
 *-----------------------------------------------------------------------------
 */
static inline uint64
cache_get_evict_interval(const cache *cc)
{
   return cc->ops->get_ei(cc);
}
...
static inline void
cache_inherit_clock(cache *cc, uint64 old_addr, uint64 new_addr)
{
   if (cc->ops->inherit_clock) {
      cc->ops->inherit_clock(cc, old_addr, new_addr, 1);
   }
}

// Inherit clock / divisor for split results.
// Each child gets old_clock / num_children.
static inline void
cache_inherit_clock_split(cache  *cc,
                          uint64  old_addr,
                          uint64  new_addr,
                          uint64  num_children)
{
   if (cc->ops->inherit_clock) {
      cc->ops->inherit_clock(cc, old_addr, new_addr, num_children);
   }
}

// Get clock value for page at addr. Returns 0 if page not in cache.
// Optionally also returns entry_time via out_entry_time (pass NULL if not needed).
static inline uint32
cache_get_page_clock(cache *cc, uint64 addr, timestamp *out_entry_time)
{
   if (cc->ops->get_page_clock) {
      return cc->ops->get_page_clock(cc, addr, out_entry_time);
   }
   if (out_entry_time) {
      *out_entry_time = 0;
   }
...
// Set clock value on a page handle.
static inline void
cache_set_page_clock(cache *cc, page_handle *page, uint32 clock)
{
   if (cc->ops->set_page_clock) {
      cc->ops->set_page_clock(cc, page, clock);
   }
}

static inline void
cache_get_lookup_timing(cache *cc, cache_lookup_timing *out)
{
   cc->ops->get_lookup_timing(cc, out);
}

static inline void
cache_set_in_lookup(cache *cc, bool32 in_lookup)
{
   if (cc->ops->set_in_lookup) {
      cc->ops->set_in_lookup(cc, in_lookup);
   }
}
