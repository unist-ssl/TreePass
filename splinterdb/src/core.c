...
core_memtable_compact(core_handle *spl, uint64 generation, const threadid tid)
{
   ...
   // Memtable CI = elapsed since root page was born / flush count.
   // Uses root page entry_time (inherited across CoW and height increases)
   // so the baseline is continuous across incorporations.
   uint64    memtable_ci_ns = 0;
   timestamp root_entry_time   = 0;
   if (spl->trunk_context.root != NULL) {
      cache_get_page_clock(
         spl->cc, spl->trunk_context.root->addr, &root_entry_time);
   }
   spl->root_ci_flush_count++;
   if (root_entry_time != 0 && comp_start > root_entry_time) {
      uint64 elapsed    = comp_start - root_entry_time;
      memtable_ci_ns    = elapsed / spl->root_ci_flush_count;

   }

   // Propagate CI to the current active memtable's mini_allocator so that
   // new memtable btree pages get CI-based clock values (prevents unnecessary
   // writeback of pages that will be discarded after incorporation).
   uint64 memtable_ei = cache_get_evict_interval(spl->cc);
   bool32 memtable_ci_le_ei =
      (TRUE
       && memtable_ci_ns > 0 && memtable_ei != 0
       && memtable_ci_ns <= memtable_ei);

   if (memtable_ci_le_ei) {
      uint64    active_gen = memtable_generation(spl->mt_ctxt);
      uint64    active_idx = active_gen % CORE_NUM_MEMTABLES;
      memtable *active_mt  = &spl->mt_ctxt->mt[active_idx];
      active_mt->mini.ci_le_ei = TRUE;
   }
   ...
   // Set the memtable CI for writeback optimization
   req.ci_le_ei = memtable_ci_le_ei;
   if (!req.ci_le_ei) {
      btree_iterator_setup_clock_accum(&btree_itor,
                                       &req.input_clock_sum,
                                       &req.input_clock_count,
                                       &req.input_clock_max);
   }
   uint64 pack_start;
...
core_lookup(core_handle *spl, key target, merge_accumulator *result)
{
   ...
   if (spl->cfg.use_stats) {
      ts = platform_get_timestamp();
      cache_set_in_lookup(spl->cc, TRUE);
   }
...
found_final_answer_early:
   ...
   if (spl->cfg.use_stats) {
      ...
      uint64 elapsed = platform_timestamp_elapsed(ts);
      spl->stats[tid].total_lookup_time_ns += elapsed;
      platform_histo_insert(spl->stats[tid].lookup_latency_histo, elapsed);
      cache_set_in_lookup(spl->cc, FALSE);
   }
...
{
   async_begin(state, 0);
   // Update thread-local timestamp for warm/cold classification.
   // In sync path this is set in splinterdb_lookup(), but async
   // path bypasses that, leaving treepass_current_ts = 0 which
   // makes all pages look COLD to btree_update_ac_and_classify_warm.
   treepass_current_ts = platform_get_timestamp();
   // look in memtables
   ...
   if (spl->cfg.use_stats) {
      ...
      for (uint64 i = 0; i < MAX_THREADS; i++) {
         ...
         rc = platform_histo_create(spl->heap_id,
                                    LATENCYHISTO_SIZE + 1,
                                    latency_histo_buckets,
                                    &spl->stats[i].lookup_latency_histo);
         platform_assert_status_ok(rc);
      }
   }
...
core_print_insertion_stats(platform_log_handle *log_handle, core_handle *spl)
{
   ...
   platform_log(log_handle, "\n");
   // Cache stats are printed in core_print_lookup_stats, skip duplicate here
   // platform_log(log_handle, "------------------------------------------------------------------------------------\n");
   ...
   if (spl->cfg.use_stats) {
      for (threadid thr_i = 0; thr_i < MAX_THREADS; thr_i++) {
         ...
         platform_histo_destroy(spl->heap_id,
                                &spl->stats[thr_i].delete_latency_histo);
         platform_histo_destroy(spl->heap_id,
                                &spl->stats[thr_i].lookup_latency_histo);
