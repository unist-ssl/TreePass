...
/*
 * Filter index entry meta helpers (matches btree pivot meta: 1+7 bit layout):
 *   meta byte = [7]=SPECIAL, [6:0]=CNT(7)
 *   stored in bits [63..56] of the uint64 index entry.
 */
static inline uint8
filter_meta_load(uint64 entry)
{
   return (uint8)(entry >> FILTER_META_SHIFT);
}

static inline bool32
filter_meta_is_special(uint64 entry)
{
   return (filter_meta_load(entry) & FILTER_META_SPECIAL) != 0;
}

static inline uint64
filter_meta_cnt(uint64 entry)
{
   return (uint64)(filter_meta_load(entry) & FILTER_META_CNT_MASK);
}

/* Mark SPECIAL bit via atomic OR */
static inline void
filter_meta_mark_special(uint64 *entry_ptr)
{
   uint64 old_val = __atomic_load_n(entry_ptr, __ATOMIC_RELAXED);
   for (;;) {
      uint64 new_val = old_val
                       | ((uint64)FILTER_META_SPECIAL << FILTER_META_SHIFT);
      if (old_val == new_val)
         return;
      if (__atomic_compare_exchange_n(entry_ptr, &old_val, new_val,
                                      false, __ATOMIC_RELAXED, __ATOMIC_RELAXED))
         return;
   }
}

/* Clear SPECIAL bit via atomic AND */
static inline void
filter_meta_clear_special(uint64 *entry_ptr)
{
   uint64 old_val = __atomic_load_n(entry_ptr, __ATOMIC_RELAXED);
   for (;;) {
      uint64 new_val = old_val
...
/* Set CNT(7) to a specific value (clamped to 0x7F). Preserves SPECIAL. */
static inline void
filter_meta_set_count(uint64 *entry_ptr, uint8 count)
{
   if (count > FILTER_META_CNT_MASK)
      count = FILTER_META_CNT_MASK;
   uint64 old_val = __atomic_load_n(entry_ptr, __ATOMIC_RELAXED);
   for (;;) {
      uint8  old_meta = (uint8)(old_val >> FILTER_META_SHIFT);
      uint8  new_meta = (old_meta & FILTER_META_SPECIAL) | count;
      uint64 new_val  = (old_val & FILTER_ADDR_MASK)
                        | ((uint64)new_meta << FILTER_META_SHIFT);
      if (__atomic_compare_exchange_n(entry_ptr, &old_val, new_val,
                                      false, __ATOMIC_RELAXED, __ATOMIC_RELAXED))
         return;
   }
...
/* Increment CNT(7) unless SPECIAL is set. When saturated, set SPECIAL. */
static inline void
filter_meta_cnt_inc(uint64 *entry_ptr)
{
   uint64 old_val = __atomic_load_n(entry_ptr, __ATOMIC_RELAXED);
   for (;;) {
      uint8 meta = (uint8)(old_val >> FILTER_META_SHIFT);
      if (meta & FILTER_META_SPECIAL)
         return; // sticky SPECIAL
      uint8 cnt      = meta & FILTER_META_CNT_MASK;
      uint8 new_meta = (cnt == FILTER_META_CNT_MASK)
                        ? FILTER_META_SPECIAL
                        : (uint8)((meta & FILTER_META_SPECIAL) | (uint8)(cnt + 1));
      uint64 new_val = (old_val & FILTER_ADDR_MASK)
                       | ((uint64)new_meta << FILTER_META_SHIFT);
      if (__atomic_compare_exchange_n(entry_ptr, &old_val, new_val,
                                      false, __ATOMIC_RELAXED, __ATOMIC_RELAXED))
                   ...
                   page_handle         **filter_page)
{
   ...
   // Read index entry and mask addr
   uint64  entry_offset = index % addrs_per_page;
   uint64 *entry_ptr    = &((uint64 *)index_page->data)[entry_offset];
   uint64  raw_entry    = __atomic_load_n(entry_ptr, __ATOMIC_RELAXED);
   uint64  hdr_raw_addr = FILTER_ADDR(raw_entry);
   platform_assert(hdr_raw_addr != 0);

   uint64 header_addr      = hdr_raw_addr - (hdr_raw_addr % page_size);
   *filter_page            = cache_get(
      cc, header_addr, TRUE, PAGE_TYPE_FILTER);
                   ...
                   bool32                ci_le_ei)
{
   ...
   // CI>EI: inherit clock from old filter's first page.
   // All pages in a filter extent are accessed together, so one page is
   // representative. Same pattern as btree leaf clock inheritance.
   uint32 inherit_clock = 0;
   if (!ci_le_ei && old_filter->addr != 0) {
      inherit_clock = cache_get_page_clock(cc, old_filter->addr, NULL);
   }
   ...
   if (ci_le_ei) {
      index_page[0] = cache_alloc_compaction_hot(cc, index_addr, PAGE_TYPE_FILTER);
   } else {
      index_page[0] = cache_alloc(cc, index_addr, PAGE_TYPE_FILTER);
      if (inherit_clock > 0)
         cache_set_page_clock(cc, index_page[0], inherit_clock);
   }
   for (uint64 i = 1; i < pages_per_extent; i++) {
      ...
      if (ci_le_ei) {
         index_page[i] = cache_alloc_compaction_hot(cc, next_index_addr, PAGE_TYPE_FILTER);
      } else {
         index_page[i] = cache_alloc(cc, next_index_addr, PAGE_TYPE_FILTER);
         if (inherit_clock > 0)
            cache_set_page_clock(cc, index_page[i], inherit_clock);
      }
   }
   filter->addr = index_addr;

   // we write to the filter with the filter cursor
   // Data pages are accessed individually (unlike index pages), so no clock inherit.
   uint64       addr          = mini_alloc(&mini, 0, NULL);
   page_handle *filter_page;
   if (ci_le_ei) {
      filter_page = cache_alloc_compaction_hot(cc, addr, PAGE_TYPE_FILTER);
   } else {
                        ...
                        uint64                num_indices)
{
        ...
        index_page_no++)
   {
      ...
      for (index_no = 0; index_no < max_index_no; index_no++) {
         uint64 hdr_raw_addr =
            FILTER_ADDR(((uint64 *)index_page->data)[index_no % addrs_per_page]);
         uint64 extent_addr =
            hdr_raw_addr
...
routing_get_header_async(routing_filter_lookup_async_state *state, uint64 depth)
{
   ...
   {
      uint64  entry_offset = state->index % state->addrs_per_page;
      uint64 *entry_ptr    = &((uint64 *)state->index_page->data)[entry_offset];
      uint64  raw_entry    = __atomic_load_n(entry_ptr, __ATOMIC_RELAXED);
      state->hdr_raw_addr  = FILTER_ADDR(raw_entry);
      filter_meta_cnt_inc(entry_ptr);
   }
                           ...
                           uint32          num_indices)
{
   ...
   for (i = 0; i < num_indices; i++) {
      ...
      platform_default_log("index 0x%lx: %lu\n",
                           i,
                           (uint64)FILTER_ADDR(((uint64 *)index_page->data)[i % addrs_per_page]));
      cache_unget(cc, index_page);
   }
}
