...
/* Filter index entry format (matches btree pivot meta: 1+7 bit layout):
 * [63]     = SPECIAL bit (was_cached flag)
 * [62..56] = access count (7 bits, 0-127)
 * [55..0]  = hdr_raw_addr (56 bits, sufficient for 64PB address space)
 */
#define FILTER_META_SHIFT       56
#define FILTER_ADDR_MASK        ((1ULL << FILTER_META_SHIFT) - 1)
#define FILTER_META_SPECIAL     (0x80u)           /* bit7 of meta byte */
#define FILTER_META_CNT_MASK    (0x7Fu)           /* bits[0..6] of meta byte */
#define FILTER_ADDR(entry)      ((entry) & FILTER_ADDR_MASK)
#define FILTER_META(entry)      ((uint8)((entry) >> FILTER_META_SHIFT))
