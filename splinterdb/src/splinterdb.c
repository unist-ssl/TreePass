...
typedef struct splinterdb {
   ...
   treepass_config  cache_cfg;
   treepass         cache_handle;
...
)
{
   ...
   treepass_config_init(&kvs->cache_cfg,
                          &kvs->io_cfg,
                          cfg.cache_size,
                          cfg.cache_logfile,
                          cfg.use_stats,
                          cfg.cache_use_stats);
   if (cfg.cache_ei_ewma_alpha > 0) {
      kvs->cache_cfg.ei_ewma_alpha = cfg.cache_ei_ewma_alpha;
   }
   if (cfg.cache_ei_initial_per_gb_ns > 0) {
      kvs->cache_cfg.ei_initial_per_gb_ns = cfg.cache_ei_initial_per_gb_ns;
   }
...
)
{
   ...
   status = treepass_init(&kvs->cache_handle,
                            &kvs->cache_cfg,
                            (io_handle *)&kvs->io_handle,
   ...
   // Initialize treepass_current_ts for the calling thread so that
   // page loads during core_mount/core_create can set entry_time.
...
deinit_cache:
   treepass_deinit(&kvs->cache_handle);
deinit_allocator:
...
)
{
   ...
   // Set current timestamp for this insert operation
   treepass_current_ts = platform_get_timestamp();
                  ...
                  splinterdb_lookup_result *result) // IN/OUT
{
   ...
   // Set current timestamp for this lookup operation
   treepass_current_ts = platform_get_timestamp();
                               ...
                               uint64                num_tuples)
{
   ...
   // Set current timestamp for cache operations during iterator init
   treepass_current_ts = platform_get_timestamp();
...
splinterdb_stats_reset(splinterdb *kvs)
{
   ...
   trunk_reset_stats(&kvs->spl->trunk_context);
}
...
core_handle *
splinterdb_get_mutable_core_handle(const splinterdb *kvs)
{
   return kvs->spl;
}
...
void
splinterdb_wait_for_quiescent(splinterdb *kvs)
{
   task_perform_until_quiescent(kvs->task_sys);
}
