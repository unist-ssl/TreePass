...
task_group_run_task(task_group *group, task *assigned_task)
{
   ...
   if (group->use_stats) {
      timestamp queue_wait_time = now - assigned_task->enqueue_time;
   ...
   // Set once per task to amortize timestamp reads for cache operations.
   treepass_current_ts = now;

   assigned_task->func(assigned_task->arg,
                       task_system_get_thread_scratch(group->ts, tid));

   if (group->use_stats) {
      timestamp elapsed = platform_timestamp_elapsed(now);
      if (elapsed > group->stats[tid].max_runtime_ns) {
         group->stats[tid].max_runtime_ns   = elapsed;
