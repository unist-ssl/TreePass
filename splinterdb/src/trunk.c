...
struct trunk_pivot {
   ...
   // Low 8 bits of child_addr encode compaction count (addresses are 4KB-aligned)
   uint64            child_addr;
   // Index of the oldest bundle that is live for this pivot
   uint64     inflight_bundle_start;
   bool32     ci_le_ei;  // CI < EI: set at serialize time using per-node entry_time
   ondisk_key key;
};
...
typedef struct bundle_compaction {
   ...
   bool32                    ci_le_ei;  // CI < EI at compaction enqueue time
} bundle_compaction;
...
/*
 * Compaction count is embedded in the low 8 bits of child_addr.
 * Trunk pages are 4KB-aligned, so bits [0..11] are free for metadata.
 * We use bits [0..7] (8 bits, 0-255) for compaction count.
 * No special bit needed (unlike btree which uses bit 7 for was_cached).
 */
#define TRUNK_PIVOT_CC_MASK       ((uint64)0xFFu)
#define TRUNK_PIVOT_ADDR_MASK           (~TRUNK_PIVOT_CC_MASK)
#define TRUNK_PIVOT_CC_SATURATION_THRESHOLD   (0xFFu)  // reset threshold

static inline uint64
trunk_pivot_child_addr(const trunk_pivot *pivot)
{
   return pivot->child_addr & TRUNK_PIVOT_ADDR_MASK;
}

static inline uint64
trunk_ondisk_pivot_child_addr(const trunk_ondisk_pivot *pivot)
{
   return pivot->child_addr & TRUNK_PIVOT_ADDR_MASK;
}

static inline uint8
trunk_pivot_cc_load(const trunk_pivot *pivot)
{
   return (uint8)(pivot->child_addr & TRUNK_PIVOT_CC_MASK);
}

static inline uint8
trunk_ondisk_pivot_cc_load(const trunk_ondisk_pivot *pivot)
{
   return (uint8)(pivot->child_addr & TRUNK_PIVOT_CC_MASK);
}

static inline void
trunk_pivot_cc_set(trunk_pivot *pivot, uint8 cnt)
{
   pivot->child_addr = (pivot->child_addr & TRUNK_PIVOT_ADDR_MASK) | cnt;
}
                   ...
                   trunk_pivot_stats stats)
{
   ...
   result->child_addr            = child_addr;  // includes cc in low byte
...
{
   trunk_pivot *dst = trunk_pivot_create(hid,
                     ...
                     platform_heap_id  hid)
{
   ...
   // Trunk node copy preserves pivot metadata (child_addr copied as-is)
                           ...
                           key              ub)
{
   ...
   trunk_node_init(
      node, 0, pivots, pivot_bundles, 0, inflight_bundles);
   return STATUS_OK;
                               ...
                               const trunk_node  *node)
{
   ...
   bool32       ret =
      trunk_pivot_child_addr(lb) == 0 && data_key_compare(data_cfg, lbkey, ubkey) < 0;
   if (!ret) {
...
/*
 * Same as trunk_ondisk_node_handle_init but reports cache miss via was_miss.
 */
static platform_status
trunk_ondisk_node_handle_init_with_miss(trunk_ondisk_node_handle *handle,
                                        cache                    *cc,
                                        uint64                    addr,
                                        bool32                   *was_miss)
{
   platform_assert(addr != 0);
   handle->cc          = cc;
   handle->header_page = cache_get_with_miss(cc, addr, TRUE, PAGE_TYPE_TRUNK,
                                             was_miss);
   if (handle->header_page == NULL) {
      platform_error_log("%s():%d: cache_get_with_miss() failed",
                         __func__, __LINE__);
      return STATUS_IO_ERROR;
   }
   handle->pivot_page           = NULL;
                                    ...
                                    uint64                          depth)
{
   ...
   platform_assert(trunk_ondisk_pivot_child_addr(state->pivot) != 0);
   state->child_handle.cc = state->context->cc;
   cache_get_async_state_init(state->cache_get_state,
                              state->context->cc,
                              trunk_ondisk_pivot_child_addr(state->pivot),
                              PAGE_TYPE_TRUNK,
                              TRUE,
                              state->callback,
                              state->callback_arg);
                        ...
                        bool32                    was_cache_miss)
{
   ...
   // On cache miss (first load from disk), clear stale compaction counter.
   // root_entry_time is reset on context init, so stale counts from a previous
   // session would produce incorrect compaction intervals.
   uint64 child_addr = was_cache_miss
                          ? (odp->child_addr & TRUNK_PIVOT_ADDR_MASK)
                          : odp->child_addr;
                       ...
                       trunk_node          *result)
{
   ...
   rc = trunk_ondisk_node_handle_init_with_miss(&handle, context->cc, addr,
                                                &was_cache_miss);
   if (!SUCCESS(rc)) {
...
trunk_ondisk_node_gc(trunk_context *context, uint64 addr)
{
   ...
   if (SUCCESS(rc)) {
      if (!trunk_node_is_leaf(&node)) {
         for (uint64 i = 0; i < vector_length(&node.pivots) - 1; i++) {
            trunk_pivot *pvt = vector_get(&node.pivots, i);
            trunk_ondisk_node_dec_ref(context, trunk_pivot_child_addr(pvt));
         }
      }
...
trunk_node_inc_all_refs(trunk_context *context, trunk_node *node)
{
   if (!trunk_node_is_leaf(node)) {
      for (uint64 i = 0; i < vector_length(&node->pivots) - 1; i++) {
         trunk_pivot *pvt = vector_get(&node->pivots, i);
         trunk_ondisk_node_inc_ref(context, trunk_pivot_child_addr(pvt));
      }
   }
   for (uint64 i = 0; i < vector_length(&node->pivot_bundles); i++) {
      bundle *bndl = vector_get_ptr(&node->pivot_bundles, i);
                ...
                trunk_ondisk_pivot *dest)
{
   ...
   dest->child_addr = pvt->child_addr;  // includes cc in low byte
...
static trunk_ondisk_node_ref *
trunk_node_serialize(trunk_context *context, trunk_node *node, uint64 old_addr)
{
   platform_status        rc;
   ...
   // Inherit clock value from the old page being replaced
   if (old_addr != 0) {
      cache_inherit_clock(context->cc, old_addr, header_addr);
   }

   // Overflow reset: if any pivot's compaction count hit 0xFF, reset all
   // pivots in this node and re-anchor entry_time (like btree does).
   // Done here while we hold header_page, avoiding a separate addr-based API.
   {
      uint64 np = vector_length(&node->pivots);
      bool32 overflow = FALSE;
      for (uint64 p = 0; p < np; p++) {
         if (trunk_pivot_cc_load(trunk_node_pivot(node, p))
             >= TRUNK_PIVOT_CC_SATURATION_THRESHOLD) {
            overflow = TRUE;
            break;
         }
      }
      if (overflow) {
         for (uint64 p = 0; p < np; p++) {
            trunk_pivot_cc_set(trunk_node_pivot(node, p), 0);
         }
         __atomic_store_n(&header_page->entry_time,
                          platform_get_timestamp(), __ATOMIC_RELAXED);
      }
   }

   // Note: entry_time is NOT persisted to disk - it's reset on deserialize
   // to track node lifetime in current execution only
                ...
                uint64                        split_source_addr)
{
   ...
   uint64 num_nodes = vector_length(nodes);
   for (uint64 i = 0; i < num_nodes; i++) {
      trunk_ondisk_node_ref *odnref =
         trunk_node_serialize(context, vector_get_ptr(nodes, i), 0);
      if (odnref == NULL) {
         platform_error_log(
      ...
      // Inherit clock / num_children from the split source node
      if (split_source_addr != 0) {
         cache_inherit_clock_split(
            context->cc, split_source_addr, odnref->addr, num_nodes);
      }

      // Compute CI<EI per pivot using this node's entry_time, now that the page
      // address is known.
      {
         trunk_node *node         = vector_get_ptr(nodes, i);
         timestamp   node_entry_time = 0;
         cache_get_page_clock(context->cc, odnref->addr, &node_entry_time);
         timestamp now = platform_get_timestamp();
         uint64    ei  = cache_get_evict_interval(context->cc);
         if (node_entry_time != 0 && ei != 0 && now > node_entry_time) {
            timestamp elapsed      = now - node_entry_time;
            uint64    num_children = trunk_node_num_children(node);
            for (uint64 p = 0; p < num_children; p++) {
               if (!trunk_node_pivot_has_received_bundles(node, p)) {
                  continue;
               }
               trunk_pivot *pvt      = trunk_node_pivot(node, p);
               uint8        cc = trunk_pivot_cc_load(pvt);
               if (cc == 0) {
                  continue;
               }
               timestamp ci  = elapsed / cc;
               pvt->ci_le_ei = (ci <= ei);
#ifdef TRUNK_COMPACTION_STATS
               uint8 height = (uint8)trunk_node_height(node);
               if (height < TRUNK_MAX_HEIGHT && context->stats) {
                  __sync_fetch_and_add(
                     &context->stats->compaction_interval_sum_ns[height], ci);
                  __sync_fetch_and_add(
                     &context->stats->compaction_interval_count[height], 1);
                  uint64 old_min =
                     context->stats->compaction_interval_min_ns[height];
                  while (old_min == 0 || ci < old_min) {
                     if (__sync_bool_compare_and_swap(
                            &context->stats
                                ->compaction_interval_min_ns[height],
                            old_min,
                            ci)) {
                        break;
                     }
                  ...
                  uint64 old_max =
                     context->stats->compaction_interval_max_ns[height];
                  while (ci > old_max) {
                     if (__sync_bool_compare_and_swap(
                            &context->stats
                                ->compaction_interval_max_ns[height],
                            old_max,
                            ci)) {
                        break;
                     }
                             ...
                             void                   *arg)
{
   ...
   trunk_ondisk_node_ref *result = NULL;
   if (SUCCESS(rc)) {
      result = trunk_node_serialize(context, &node, addr);
   }
                                      ...
                                      void          *arg)
{
   ...
   for (uint64 i = 0; i < trunk_node_num_children(target); i++) {
      ...
      if (pivot_matches_compaction(context, target, i, args)) {
         ...
         static uint32_t apply_seq = 0;
         uint32_t seq = __sync_fetch_and_add(&apply_seq, 1);

         if (seq < 0) {
            uint64 child_addr = trunk_pivot_child_addr(pvt);
            platform_error_log("[APPLY_SEQ_%u] BEFORE cnt_inc: addr=%lu\n", seq, child_addr);
         }
         ...
         // Increment compaction counter (saturates at TRUNK_PIVOT_CC_SATURATION_THRESHOLD;
         // overflow reset happens in serialize_nodes when the node is written)
         uint8 cnt = trunk_pivot_cc_load(pvt);
         if (cnt < TRUNK_PIVOT_CC_SATURATION_THRESHOLD) {
            trunk_pivot_cc_set(pvt, cnt + 1);
         }

         args->found_match = TRUE;

         if (seq < 0) {
            platform_error_log("[APPLY_SEQ_%u] DONE\n", seq);
         }
         break;
      }
   }
...
maplet_compaction_task(void *arg, void *scratch)
{
   ...
   while (bc != NULL && bc->state == BUNDLE_COMPACTION_SUCCEEDED) {
      if (!branch_is_null(bc->output_branch)) {
         ...
         bool32 maplet_ci_le_ei = bc->ci_le_ei;
...
bundle_compaction_task(void *arg, void *scratch)
{
   ...
   if (state->abandoned) {
      // Save height BEFORE release_entry: if release decrements refcount to
      // 0, trunk_pivot_state_destroy frees state, making state->height a
      ...
      if (context->stats) {
         context->stats[tid].compactions_aborted[height]++;
      }
      return;
   }
   ...
   // CI <= EI: alloc sets max clock, skip inherit overhead
   pack_req.ci_le_ei = bc->ci_le_ei;



   // Clock accumulation only needed when CI > EI (inherit path).
   for (uint64 i = 0; i < vector_length(&merger.itors); i++) {
      btree_iterator *bitor =
         (btree_iterator *)vector_get(&merger.itors, i);
      bitor->is_warm = FALSE;
      if (!pack_req.ci_le_ei) {
         btree_iterator_setup_clock_accum(
            bitor, &pack_req.input_clock_sum, &pack_req.input_clock_count,
            &pack_req.input_clock_max);
      }
   }
...
{
   // Truncate any leftover data from previous incorporation
   // (should be empty, but defensive programming)
   uint64 len = vector_length(&itasks->node_compactions);
   if (len > 0) {
      vector_truncate(&itasks->node_compactions, 0);
   }
   ...
   uint64                        split_source_addr)
{
   ...
   rc = serialize_nodes(context, nodes, result, split_source_addr);
   if (!SUCCESS(rc)) {
      platform_error_log("serialize_nodes_and_save_contingent_compactions: "
                          ...
                          uint64        *estimate)
{
   ...
   assert(trunk_node_is_well_formed_leaf(context->cfg->data_cfg, leaf));
           ...
           bool32            *abandon_compactions)
{
   ...
   for (uint64 i = 0; i < vector_length(&pivots) - 1; i++) {
      ...
      assert(trunk_node_is_well_formed_leaf(
...
{
   assert(trunk_node_is_well_formed_index(context->cfg->data_cfg, index));
   platform_status rc;
   ...
   for (uint64 i = 0; i < num_nodes; i++) {
      ...
      assert(trunk_node_is_well_formed_index(
                     ...
                     uint64                        old_addr)
{
   ...
   rc = serialize_nodes_and_save_contingent_compactions(
      context, &new_nodes, new_leaf_refs, itasks, old_addr);
   if (!SUCCESS(rc)) {
                   ...
                   incorporation_tasks          *itasks)
{
   ...
   // Save parent's compaction count to inherit to new children
   uint8 parent_cc = trunk_pivot_cc_load(pvt);
   ...
   // Split parent's compaction count among children
   uint64 num_children = vector_length(&new_pivots);
   uint8 child_cc = (num_children > 0) ? (parent_cc / num_children) : 0;

   for (uint64 j = 0; j < num_children; j++) {
      trunk_pivot *new_pivot = vector_get(&new_pivots, j);
      trunk_pivot_set_inflight_bundle_start(
         new_pivot, vector_length(&index->inflight_bundles));
      trunk_pivot_cc_set(new_pivot, child_cc);
   }
                      ...
                      uint64                        old_addr)
{
   ...
   rc = serialize_nodes_and_save_contingent_compactions(
      context, &new_nodes, new_index_refs, itasks, old_addr);
   if (!SUCCESS(rc)) {
      platform_error_log(
                   ...
                   uint64                        old_addr)
{
   ...
   if (trunk_node_is_leaf(node)) {
      rc = restore_balance_leaf(context, node, new_node_refs, itasks, old_addr);
   } else {
      rc = restore_balance_index(context, node, new_node_refs, itasks, old_addr);
   }
                ...
                trunk_ondisk_node_ref_vector *node_refs)
{
   ...
   assert(1 < vector_length(node_refs));
   ...
   // Inherit clock/entry_time from first child so new root's CI baseline is
   // continuous (child already inherited entry_time from old root via split).
   uint64 child_addr = vector_get(node_refs, 0)->addr;
   rc = serialize_nodes(context, &new_nodes, &new_ondisk_node_refs, child_addr);
   VECTOR_APPLY_TO_PTRS(&new_nodes, trunk_node_deinit, context);
   vector_deinit(&new_nodes);
   if (!SUCCESS(rc)) {
...
trunk_incorporate_prepare(trunk_context *context, uint64 branch_addr)
{
   ...
   // IMPORTANT: Each incorporation must have its own tasks structure to avoid
   // concurrent access from multiple threads doing simultaneous incorporations
   ...
   uint64 root_addr = context->root ? context->root->addr : 0;
                   ...
                   platform_log_handle      *log)
{
   ...
   while (handlep && handlep->header_page) {
      ...
      // Search the child
      if (trunk_ondisk_pivot_child_addr(pivot) != 0) {
         trunk_ondisk_node_handle child_handle;
         rc = trunk_ondisk_node_handle_init(
            &child_handle, context->cc, trunk_ondisk_pivot_child_addr(pivot));
         if (!SUCCESS(rc)) {
            platform_error_log("trunk_merge_lookup: "
...
trunk_merge_lookup_async(trunk_merge_lookup_async_state *state)
{
   ...
   while (state->handlep && state->handlep->header_page) {
      ...
      // Search the child
      if (trunk_ondisk_pivot_child_addr(state->pivot) != 0) {
                       ...
                       key_buffer                     *max_key)
{
   ...
   while (handle.header_page) {
      ...
      child_addr           = trunk_ondisk_pivot_child_addr(pivot);
      ...
      if (child_addr != 0) {
         ...
         key leaf_min_key;
         key leaf_max_key;
         assert(trunk_ondisk_node_num_pivots(&handle) == 2);
...
#ifdef TRUNK_COMPACTION_STATS
   STATS_FIELD_ADD(dst, src, compaction_interval_sum_ns);
   STATS_FIELD_ADD(dst, src, compaction_interval_count);
   for (uint32 h = 0; h < TRUNK_MAX_HEIGHT; h++) {
      if (src->compaction_interval_min_ns[h] > 0 &&
          (dst->compaction_interval_min_ns[h] == 0 ||
           src->compaction_interval_min_ns[h] <
              dst->compaction_interval_min_ns[h])) {
         dst->compaction_interval_min_ns[h] = src->compaction_interval_min_ns[h];
      }
   }
   STATS_FIELD_MAX(dst, src, compaction_interval_max_ns);
#endif // TRUNK_COMPACTION_STATS
...
#ifdef TRUNK_COMPACTION_STATS
   //
   // Trunk Compaction Interval Statistics
   //
   platform_log(log_handle,
                "\nTrunk Compaction Interval Statistics:\n");
   platform_log(log_handle,
                "Height | Count      | Avg CI (s) | Min CI (s) | Max CI (s)\n");
   platform_log(log_handle,
                "-------|------------|------------|------------|------------\n");
   for (int32 h = (height < TRUNK_MAX_HEIGHT ? height : TRUNK_MAX_HEIGHT - 1);
        h >= 0;
        h--)
   {
      if (global_stats.compaction_interval_count[h] > 0) {
         double avg_ci_s = (double)global_stats.compaction_interval_sum_ns[h]
                           / global_stats.compaction_interval_count[h] / 1e9;
         double min_ci_s =
            (double)global_stats.compaction_interval_min_ns[h] / 1e9;
         double max_ci_s =
            (double)global_stats.compaction_interval_max_ns[h] / 1e9;
         platform_log(log_handle,
                      "%6d | %10lu | %10.3f | %10.3f | %10.3f\n",
                      h,
                      global_stats.compaction_interval_count[h],
                      avg_ci_s,
                      min_ci_s,
                      max_ci_s);
      }
                     ...
                     void          *arg)
{
   ...
   for (int i = 0; i < trunk_node_num_children(node); i++) {
      ...
      rc    = trunk_node_deserialize(context, trunk_pivot_child_addr(pivot), &child);
      if (!SUCCESS(rc)) {
...
/*
 * Branch warmth CSV: walk the trunk tree and collect per-branch-tree
 * pivot counter stats.  Output one CSV row per branch tree.
 *
 * Columns:
 *   trunk_height, pivot_idx, bundle_idx, branch_idx,
 *   btree_height, total_pivots, special, active,
 *   special_pct, active_cv, combined_cv, leaf_pages, internal_pages
 */
typedef struct branch_warmth_ctx {
   platform_log_handle *log;
   trunk_context       *context;
} branch_warmth_ctx;

static platform_status
branch_warmth_visitor(trunk_context *context,
                      trunk_node    *node,
                      void          *arg)
{
   branch_warmth_ctx *ctx = (branch_warmth_ctx *)arg;
   uint16 h = node->height;

   const data_config *data_cfg = context->cfg->data_cfg;
   uint64 num_children = trunk_node_num_children(node);
   for (uint64 i = 0; i < num_children; i++) {
      key pivot_key = trunk_node_pivot_key(node, i);
      const bundle *bndl = trunk_node_pivot_bundle(node, i);
      uint64 num_branches = bundle_num_branches(bndl);
      const branch_ref *branches = bundle_branch_array(bndl);

      for (uint64 j = 0; j < num_branches; j++) {
         uint64 addr = branch_ref_addr(branches[j]);
         btree_warmth_stats stats;
         btree_collect_warmth_stats(context->cc,
                                   context->cfg->btree_cfg,
                                   addr,
                                   PAGE_TYPE_BRANCH,
                                   &stats);
         double special_pct = stats.total_pivots > 0
            ? 100.0 * stats.special_count / stats.total_pivots : 0;
         uint64 cached_total = stats.cached_leaf_pages
                             + stats.cached_internal_pages;

         platform_log(ctx->log,
                      "%u,%s,%lu,"
                      "%lu,%lu,%lu,"
                      "%.1f,%.3f,%.3f,"
                      "%lu,%lu,%lu\n",
                      h,
                      key_string(data_cfg, pivot_key),
                      j,
                      stats.total_pivots,
                      stats.special_count,
                      stats.active_count,
...
void
trunk_print_branch_warmth_csv(platform_log_handle *log_handle,
                              trunk_context       *context)
{
   if (context->root == NULL) {
      platform_log(log_handle, "[BRANCH-WARMTH] No trunk tree\n");
      return;
   }

   platform_log(log_handle,
                "[BRANCH-WARMTH-CSV-BEGIN]\n"
                "trunk_height,pivot_key,branch_idx,"
                "total_pivots,special,active,"
                "special_pct,active_cv,combined_cv,"
                "cached_leaf,cached_internal,cached_total\n");

   branch_warmth_ctx ctx = {
      .log     = log_handle,
      .context = context,
   };

   platform_status rc = visit_nodes(context, branch_warmth_visitor, &ctx);
   if (!SUCCESS(rc)) {
      platform_log(log_handle,
                   "[BRANCH-WARMTH] visit_nodes failed: %d\n", rc.r);
   }
   platform_log(log_handle, "[BRANCH-WARMTH-CSV-END]\n");
}
