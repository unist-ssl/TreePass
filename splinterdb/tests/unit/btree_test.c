...
CTEST_SETUP(btree)
{
   ...
   if (!init_data_config_from_master_config(data->data_cfg, &data->master_cfg)
       ...
       || !init_treepass_config_from_master_config(
...
index_hdr_tests(btree_config *cfg, btree_scratch *scratch, platform_heap_id hid)
{
   ...
   /* child_addr must be 4KiB-aligned; TreePass encodes pivot metadata in the
    * low byte, so use (i << 12) to produce properly-aligned test addresses. */
   for (uint32 i = 0; i < nkvs; i++) {
      rv = btree_set_index_entry(
         cfg, hdr, i, key_create(i % sizeof(i), &i), (uint64)i << 12, stats);
      ASSERT_TRUE(rv, "Failed to insert 4-byte %d\n", i);
   }
   ...
   for (uint32 i = 0; i < nkvs; i++) {
      ...
      ASSERT_EQUAL(childaddr, (uint64)i << 12, "Bad childaddr %d\n", i);
   }

   for (uint64 i = 0; i < nkvs; i++) {
      rv = btree_set_index_entry(
         cfg, hdr, i, key_create(i % sizeof(i), &i), i << 12, stats);
      ASSERT_TRUE(rv, "Failed to insert 8-byte %ld\n", i);
   }
   ...
   for (uint64 i = 0; i < nkvs; i++) {
      ...
      ASSERT_EQUAL(childaddr, i << 12, "Bad childaddr %d\n", i);
   }
